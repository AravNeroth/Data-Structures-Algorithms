...
public class GameBoard extends Canvas implements MouseListener {
	...
	public boolean determineWinner(Graphics window) {
		...
		if (winner.equals("")) {
			...
			// yes i know there's a better way to do this, but this is already late
			// so i just hard coded it for diagnoally

			Piece mark1 = (Piece) board.getSpot(0, 0);
			Piece mark2 = (Piece) board.getSpot(1, 1);
			Piece mark3 = (Piece) board.getSpot(2, 2);

			Piece mark4 = (Piece) board.getSpot(0, 2);
			Piece mark5 = (Piece) board.getSpot(1, 1);
			Piece mark6 = (Piece) board.getSpot(2, 0);

			if (mark1.getName().equals(mark2.getName()) && mark1.getName().equals(mark3.getName())) {
				winner = mark1.getName() + " wins diagonally!";
			}
			else if (mark4.getName().equals(mark5.getName()) && mark4.getName().equals(mark6.getName())) {
				winner = mark1.getName() + " wins diagonally!";
			}
		}
